...
func wander():
	if workstation_near:
		var vectorpos = (position-self.translation).normalized()
...
func Spatial_move_to(vector,delta):
	...
	if (get_parent().is_on_floor()): #Only lets the character change it's facing direction when it's on the floor.
...
func AI_is_seeing():
	if visible.is_colliding():
		...
		for x in obj_seen_grps:
			...
			if x == "Player" and not visible.get_collider().Team == Team:
				Spatial_move_to(self.translation - visible.get_collider().translation)
				has_target = true
			#else:
