...
func _ready():
	...
	if health >= 101:
		$"Health|Radar/x2".visible = true
	if health >= 201:
		$"Health|Radar/x3".visible = true
	change_health()
...
func change_health():
	...
	if health >= 1 and health <= 100:
		get_node("Health|Radar/x1/T_health").interpolate_property(get_node("Health|Radar/x1"),"indicator_value", null ,float(health)/100 ,1.5,Tween.TRANS_LINEAR,Tween.EASE_OUT,0)
		#.indicator_value = float(health)/100
	elif health <= 0:
		get_node("Health|Radar/x1/T_health").interpolate_property(get_node("Health|Radar/x1"),"indicator_value", null ,0  ,1.5,Tween.TRANS_LINEAR,Tween.EASE_OUT,0)
	elif health >= 100:
		get_node("Health|Radar/x1/T_health").interpolate_property(get_node("Health|Radar/x1"),"indicator_value", null ,1 ,1.5,Tween.TRANS_LINEAR,Tween.EASE_OUT,0)
